...
class Colors:
    def __init__(self):
        ...
        hex = ('FF9D97', 'FF3838',  'FF701F', 'FFB21D', 'CFD231', '48F90A', '92CC17', '3DDB86', '1A9334', '00D4BB',
...
class BboxAnnotator:
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=False):
        ...
        if self.pil:  # use PIL
            ...
            self.font = ImageFont.truetype(font_path, font_size or max(round(sum(self.im.size) / 2 * 0.035), 12))
        else:  # use cv2
            self.im = im
    ...
    def text_pil(self, xy, text, txt_color=(255, 255, 255)):
        ...
        '''
        if self.pil:
            self.draw.text((xy[0], xy[1]), text, fill=txt_color, font=self.font, anchor='ls')  # for PIL>8.0
    ...
    def text_cv(self, box, text, color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        p2 = p1[0] + w, p1[1] - h - 3 if outside else p1[1] + h + 3
        cv2.rectangle(self.im, p1, p2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(self.im, text, (p1[0], p1[1] - 2 if outside else p1[1] + h + 2), 0, self.lw / 3, txt_color,
                    thickness=tf, lineType=cv2.LINE_AA)
...
def bbox_color(image_input_path, label_input_path, output_path):
    ...
    for label in tqdm(labels, desc='Drawing the bbox to images and save the images'):
        ...
        Image.fromarray(im0).save(output_path.joinpath(label.stem + '.jpg')) # 根据情况选择保存的后缀
    logging.info('All Finish! (*￣︶￣) ,233~')

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Draw the bounding box to image for checking!", epilog="Color means much! Really~")
    parser.add_argument('-i', '--image_input_path', type=str, required=True, help='the path of image', metavar='image_input_path') # For IDE
    parser.add_argument('-l', '--label_input_path', type=str, required=True, help='the path of label! the format is YOLO(cxywh)', metavar='label_input_path')
    ...
    parser.add_argument('-o', '--output_path', type=str, default='./bbox_images', help='the path to get bounding box image!', metavar='bbox_output_path')
