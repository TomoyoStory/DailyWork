...
def object_split_train_val_dataset(imgs_input_path: str, 
    ...
    for x in tqdm(train_indices, desc='Copying Train Dataset', unit='batchs'):
        ...
        dst_label = output_path.joinpath('train').joinpath('labels').joinpath(src_label.name)
        src_image = imgs_input_path.joinpath(src_label.stem + '.jpg') #^ 图像格式后缀根据情况修改
        dst_image = output_path.joinpath('train').joinpath('images').joinpath(src_label.stem + '.jpg')
    ...
    for x in tqdm(val_indices, desc='Copying Val Dataset', unit='batchs'):
        src_label = labels_list[x]
        dst_label = output_path.joinpath('val').joinpath('labels').joinpath(src_label.name)
        src_image = imgs_input_path.joinpath(src_label.stem + '.jpg') #^ 图像格式后缀根据情况修改
        dst_image = output_path.joinpath('val').joinpath('images').joinpath(src_label.stem + '.jpg')
        shutil.copy(src_label, dst_label)
        shutil.copy(src_image, dst_image)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--imgs_input_path', type=str, default='./images', help='the inputpath of source image', metavar='imgs_input_path') # For IDE
    parser.add_argument('-l', '--labels_input_path', type=str, default='./labels', help='the inputpath of labels!', metavar='labels_input_path')
    parser.add_argument('-o', '--output_path', type=str, default='./DataSet', help='the outpath of dataset dir', metavar='dataset_output_path')
    parser.add_argument('-ts', '--train_sacle', type=float, default=0.85, help='the scale of train dataset in all data!', metavar='train_sacle')
    opt = parser.parse_args()

    object_split_train_val_dataset(opt.imgs_input_path, opt.labels_input_path, opt.output_path, opt.train_sacle)