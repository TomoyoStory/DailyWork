...
def get_BSTLD_categorys(
    input_path: str,
    output_path: str,
    BSTLD_labels_path: list = ["train.yaml", "test.yaml", "additional_train.yaml"],
) -> dict:
    """
    ...
    for label_file in BSTLD_labels_path:
        ...
        with open(input_path.joinpath(label_file), "r", encoding="utf-8") as f:
            ...
            logging.info(
                "Reading %s is finished" % str(input_path.joinpath(label_file))
            )
            for label in tqdm(
                labels,
                desc="Counting %s dataset categories and number" % label_file,
                unit=" images",
            ):
    ...
    logging.info(
        "Writing Bosch Small Traffic Lights Dataset number of categories to %s"
        % str(output_path.joinpath("categorys_count.txt"))
    )
    # 类别个数统计输出
    ...
    logging.info(
        "writing BSTLD.names file to %s" % str(output_path.joinpath("BSTLD.names"))
    )
    # 类别名称和ID输出
    keys_dict = {}
...
def BSTLD_to_yolo(
    ...
    for label_file in BSTLD_labels_path:
        ...
        # 构建每个训练集类别的文件目录
        current_file_image_output_path = image_output_path.joinpath(
            Path(label_file).stem
        )
        current_file_label_output_path = label_output_path.joinpath(
            Path(label_file).stem
        )
        ...
        with open(input_path.joinpath(label_file), "r", encoding="utf-8") as f:
            ...
            for label in tqdm(
                labels,
                desc="Changing %s's BSTLD format to yolo format and Copying the images which have trffic light"
                % label_file,
                unit=" images",
            ):
                label_str = ""  # ^ 标签对应字段
                for current_label in label["boxes"]:
                    ...
                    x_center = (
                        (current_label["x_max"] + current_label["x_min"]) / 2
                    ) / width
                    y_center = (
                        (current_label["y_max"] + current_label["y_min"]) / 2
                    ) / height
                ...
                if len(label["boxes"]) != 0:
                    if label_file == "test.yaml":
                        image_path = (
                            input_path.joinpath("rgb")
                            .joinpath("test")
                            .joinpath(Path(label["path"]).name)
                        )  #! 需要特别注意的是,test.yaml的路径格式和其他两个不一样,给的是url地址,所以这里需要根据实际的情况进行转换
                    else:
                        image_path = input_path.joinpath(label["path"])  # ^ 当前对应图像的绝对路径

                    with open(
                        current_file_label_output_path.joinpath(
                            image_path.stem + ".txt"
                        ),
                        "w",
                        encoding="utf-8",
                    ...
                    shutil.copy(
                        image_path,
                        current_file_image_output_path.joinpath(image_path.name),
                    )  # ^ 复制图片
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Change Bosch Small Traffic Lights Dataset format to yolo dataset format!",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="the root dir of Bosch Small Traffic Lights Dataset!",
        metavar="BSTLD_root_dir",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="the label of Bosch Small Traffic Lights Dataset! (relative path)",
        metavar="BSTLD_label_path",
    )
    ...
    parser.add_argument(
        ...
        "--output_path",
        type=str,
        default="/data/Bosch_Small_Traffic_Lights",
        help="the file output path of Bosch Small Traffic Lights Dataset format to yolov5 format!",
        metavar="BSTLD_output_path",
    )
