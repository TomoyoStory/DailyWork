...
def coco_to_yolo(coco_image_path: str,
                 coco_label_path: str,
    ...
    for category in tqdm(COCO_CLASS, desc='Coping images and Writing labels', unit='category'):
        # logging.info('Writing Class ---%s--- to yolo format' % category)
        category_id = coco.getCatIds(category)
        category_number.update({category:str(len(coco.getAnnIds(catIds=category_id)))}) #^ 类别个数统计
        img_ids = coco.getImgIds(catIds=category_id)
        imgs = coco.loadImgs(img_ids)
        for img in imgs:
            file_str = '' #^ 保存当前类别字段
            annIds = coco.getAnnIds(imgIds=img['id'], catIds=category_id, iscrowd=None) #^ iscrowd属性根据实际情况自行选择
            ...
            for ann in anns:
                ...
                file_str = file_str + str(categorys_dict[category]) + ' '+ ' '.join(("%.6f" % x_center,"%.6f" % y_center,"%.6f" % w,"%.6f" % h)) + '\n'
            yolo_output_label_name = label_output_path.joinpath(Path(img['file_name']).stem + '.txt' )
            # 追加写标签
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Change COCO dataset format to yolo dataset format! Please select the CLASSES in PYTHON FILE for your class!", epilog="Hello! AI dataset! Hummmm....")
    ...
    parser.add_argument('-i', '--image_path', type=str, default='./train2017', help='the image path of COCO dataset!', metavar='coco_image_path')  # FOR IDE
    parser.add_argument('-l', '--label_path', type=str, default='./annotations_trainval2017/annotations/instances_train2017.json', help='the label path of COCO dataset!', metavar='coco_label_path')  # FOR IDE
    parser.add_argument('-o', '--output_path', type=str, default='./data/train', help='the file output path of COCO format to yolov5 format!', metavar='coco_output_path')
