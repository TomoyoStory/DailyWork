...
def voc2yolo(
    ...
    """
    for file in tqdm(
        os.listdir(input_path), desc="Changing VOC format to YOLO format!", unit="xmls"
    ):
        label_file = input_path + os.sep + file
        if (
            os.path.isfile(label_file) and Path(label_file).suffix.lower()[1:] == "xml"
        ):  # 指定仅读取xml
            with open(
                output_path + os.sep + file.replace("xml", "txt"), "w"
            ) as out_file:
                ...
                )  #! 原始VOC中宽高可能存在为零的问题,通过自行设置width和height覆盖解决这个问题
                ...
                for obj in root.iter("object"):
                    ...
                    out_file.write(
                        str(cls_id) + " " + " ".join("%.6f" % x for x in bbox) + "\n"
                    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Change VOC dataset format to yolo dataset format! Please Change the CLASSES in PYTHON FILE for your class!",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="the file input path of VOC format to yolov5 format!",
        metavar="voc_input_path",
    )
    parser.add_argument(
        "-o",
        "--output_path",
        type=str,
        default="./yolo_format",
        help="the file output path of VOC format to yolov5 format!",
        metavar="voc_output_path",
    )
    parser.add_argument(
        "-W",
        "--width",
        type=int,
        help="the image width, it will cover the width in xml file!",
        metavar="cover_width",
    )
    parser.add_argument(
        "-H",
        "--height",
        type=int,
        help="the image height, it will cover the height in xml file!",
        metavar="cover_height",
    )
    # 便于在IDE中运行
