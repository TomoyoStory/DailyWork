...
def cluster_yolo2anchor(
    ...
    bbox = []
    for file in tqdm(
        list(label_path.iterdir()),
        desc=f"Reading file from {str(label_path)}",
        unit="files",
    ):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Use Kmeans to cluster the Yolo format files to get Anchors",
        epilog="Anchor based style~ Emmmmmm...",
    )
    parser.add_argument(
        "-p",
        "--label_path",
        type=str,
        required=True,
        help="Yolo dataset label path",
        metavar="yolo_lable_path",
    )
    parser.add_argument(
        "-o",
        "--output_anchor_file",
        type=str,
        default="./yolo_anchor.txt",
        help="output file name",
        metavar="output_anchor_name",
    )
    parser.add_argument(
        "-wi",
        "--width",
    ...
    parser.add_argument(
        ...
        type=str,
        default="k-means++",
        help='Kmeans init, "random" or "k-means++"(default)',
        metavar="kmeans_init",
    )
